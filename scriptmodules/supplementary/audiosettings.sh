...
rp_module_flags="!all rpi armbian"
...
function gui_audiosettings() {
    ...
    if [[ `aplay -ql | grep -e bcm2835 -e vc4hdmi | wc -l` < 1 ]]; then
        if [[ `aplay -ql | wc -l` < 1 ]]; then
            ...
            if _pa_cmd_audiosettings systemctl -q --user is-enabled pulseaudio.service; then
                _pulseaudio_armbian_audiosettings
            else
                _alsa_internal_armbian_audiosettings
            fi
            return
        fi
    fi
...
function _alsa_internal_armbian_audiosettings() {
    ...
    if [[ -n "$choice" ]]; then
        case "$choice" in
            [0-9])
                _asoundrc_save_armbian_audiosettings $choice ${options[$((choice*2+1))]}
...
function _asoundrc_save_armbian_audiosettings() {
    [[ -z "$1" ]] && return
...
function _pulseaudio_armbian_audiosettings() {
    local options=()
    local sinks=()
    local sink_index
    local sink_label
